...
export async function getHomeData() {
  ...
  const [heroRes, servicesRes, projectsRes, blogRes, expRes, hwwRes, partnersRes, clientLogosRes] = await Promise.allSettled([
    // 1. Hero (Home) - Fetch all fields to avoid column mismatch
    pool.query(`
      SELECT h.*,
             COALESCE(json_agg(DISTINCT jsonb_build_object('id', hm.id, 'url', mi.url, 'alt', mi.alt_text, 'variant', hm.variant)) FILTER (WHERE hm.id IS NOT NULL), '[]'::json) as media,
             COALESCE(json_agg(DISTINCT jsonb_build_object('id', hb.id, 'text', hb.text, 'icon', hb.icon, 'sort_order', hb.sort_order)) FILTER (WHERE hb.id IS NOT NULL), '[]'::json) as bullets
